...
var ExtensionManager = (function(root,loader){
   //check the root files
      var root = root,
            ...
            //root.extensionName = "ExtensionManager";
     ...
     iterators = function(collection,eachfunc,finish){
      ...
      validateDependency = function(e,dep){
           if(!e || dep.length <= 0) return [];
            ...
            for(; i < len; i++){
               item = dep[i];
               if(matchType(item,"string") && !e[item]) {
                throw new Error("Dependency "+item+" does not exist on object "+ (e.name || e.extensionName));
               }
