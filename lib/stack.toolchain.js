...
})("ToolChain",function(ToolStack){

      ToolStack.ToolChain = {
   
         //meta_data
           name:"ToolStack.ToolChain",
         ...
         //use to match arrays to arrays to ensure values are equal to each other,
           ...
           //for string just iterates a single character as specificed in the first arguments 
           ...
           //allows the breaking of the loop in its state by passing an addition function
           ...
           // if the loop is broken,though node that the completed function in eachParrallel
           // which runs using forEach will not run
           ...
           //      var counter = 0;
           //      if(finishfn) counter = this.keys(obj).length;
                 
           //      for(var e in obj){
           //          callback.call(scope || this,obj[e],e,obj);
           //          if(breakerfunc && (breakerfunc.call(scope,obj[i],i,obj))) break;
           //          if(finishfn){
           //            if(--counter === 0) finishfn.call(scope);
           //          }
           //      }
           //       return true;
           //     }
              ...
              if(!obj.length) return false;
                 ...
                 for(; i < len; i++){
                    if(breakerfunc && (breakerfunc.call((scope || this),obj[i],i,obj))){
                      if(complete) complete.call((scope || this)); break;
                    }
                    (function(z,a,b,c){
           ...
           eachAsync: function(obj,iterator,complete,scope,breaker){
              ...
              this.forEach(obj,function(x,i,o){
                iterator(x,i,o,function(err){
                    if(err){
                      complete.call((scope || this),err);
                      complete = function(){};
                    }else{
                      step -= 1;
                      if(step === 0) complete.call((scope || this));
                    }
                });
           ...
           eachSync: function(obj,iterator,complete,scope,breaker){
              ...
              fuse = function(){
                ...
                (function(z,a,b,c){
                  ...
                  iterator.call(z,a,b,c,function(err){
                      if(err){
                        complete.call(z,err);
                        complete = function(){};
                      }else{
                        step += 1;
                        if(step === keys.length) complete.call(z);
                        else fuse();
                      }
                  });
